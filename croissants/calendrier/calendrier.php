...
/***************************************************************************
	...
	DL sur http://www.phptools4u.com/scripts/calendrier/exemples.php
***************************************************************************/
...
function calendar($date = '') {
	...
	$param_d['link_on_day']			= $PHP_SELF.'?page=liste&date=%%dd%%'; // Link to put on each day
    ...
    //$date = priv_reg_glob_calendar('date'); // Modif by LCO
	if ($date == '') {
		$timestamp = time();
	}
	else {
	...
	if ( $param['day_mode'] == 1 ) {
		...
		for ($i = 1; $i <= $nb_days_month; $i++) {
			...
			elseif ($param['link_on_day'] != '') {
				...
				/*if (( ($param['link_after_date'] == 0) && ($current_timestamp < $loop_timestamp)) || (($param['link_before_date'] == 0) && ($current_timestamp >= $loop_timestamp)) ){  // Modif by LCO
	...
	if ($param['nav_link'] == 1) {
		...
		if ( ($param['link_after_date'] == 0) 
			...
			$next_day_link 		= '<a href="'.$PHP_SELF.'?page=liste&date='.$next_day.'" title="'.$calendar_txt[$param['lang']]['misc'][3].'">'.$d.'</a>'."\n";
		}

		/*if ( ($param['link_before_date'] == 0)  // Modif by LCO
				&& ($current_timestamp > mktime(0,0,0, $current_month, $current_day-1, $current_year))
			){
			$previous_day_link = '&nbsp;';
		}
		else {*/
			$previous_day_link 		= '<a href="'.$PHP_SELF.'?page=liste&date='.$previous_day.'" title="'.$calendar_txt[$param['lang']]['misc'][2].'">'.$g.'</a>'."\n";
		//}
		...
		if ( ($param['link_after_date'] == 0) 
			...
			$next_month_link 	= '<a href="'.$PHP_SELF.'?page=liste&date='.$next_month.'" title="'.$calendar_txt[$param['lang']]['misc'][1].'">'.$dd.'</a>'."\n";
		}
		...
		/*if  ( ($param['link_before_date'] == 0)  // Modif by LCO
			...
			$previous_month_link 	= '<a href="'.$PHP_SELF.'?page=liste&date='.$previous_month.'" title="'.$calendar_txt[$param['lang']]['misc'][0].'">'.$gg.'</a>'."\n";
		//}
